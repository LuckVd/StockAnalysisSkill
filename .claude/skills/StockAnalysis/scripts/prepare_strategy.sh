...
if [ "$#" -lt 2 ]; then
  echo "用法: prepare_strategy.sh 股票代码 策略名 [市场]" >&2
  exit 1
fi
...
"$PYTHON_BIN" "$SCRIPT_DIR/build_analysis_context.py" --mode strategy --input "$TMP_DIR/normalized.json" --symbol "$SYMBOL" --market "$MARKET" --strategy "$STRATEGY" > "$TMP_DIR/context.json"
"$PYTHON_BIN" "$SCRIPT_DIR/build_host_prompt.py" --mode strategy --context "$TMP_DIR/context.json"
